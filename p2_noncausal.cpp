...
// Citation : This code is referred from stackoverflow (stackoverflow.com/questions/7868936/read-file-line-by-line-using-ifstream-in-c)   			
			...
			cout << "Message Send by the Process: " << receiver_message << endl;
...
{	
	...
	//Allowing multiple sockets to use the same address
	...
	if(pthread_create(&sender_thread,NULL,sender,&order)<0)
	{
	  cout << "Error in creating a thread for sender" << endl;
	  exit(1);
	}
	if(pthread_create(&receiver_thread,NULL,receiver,&order)<0)
	{
